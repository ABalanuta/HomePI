...
class BTDetector(Thread):
	...
	def __init__(self, hub, db=True):
		...
		self.hub = hub
		self.db = db
		self.presence = MongoClient().test_database.presence
	...
	def run(self):
		...
		while not self.stopped:
			...
			if isinstance(exe.value, list):
				...
				if len(exe.value) == 0 and self.isFailedInterface():
					self.reset_interface()
				...
				for device in exe.value:
					log = { "date": device['LastSeen'],
						"name": device['Name'],
						"mac" : device['Address'],
						"rssi": device['RSSI'],
						"quality" : device['Quality']}
					self.presence.insert(log)
	...
	def isFailedInterface(self):
		p = subprocess.Popen('sudo hciconfig', shell=True, stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
